...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await app_1.pool.query(query, params);
...
router.get('/date/:date', async (req, res) => {
    try {
        ...
        const result = await app_1.pool.query(query, [date]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await app_1.pool.query(`INSERT INTO records (date, scheme_id, scheme_name, meals, note)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING ${RECORD_FIELDS}`, [date, schemeId || null, schemeName || '手动记录', JSON.stringify(meals), note || null]);
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await app_1.pool.query('DELETE FROM records WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json(error('记录不存在'));
        }
